...
<template>
  <NuxtLink :to="`/products/${product.id}`">
    <div
      class="cursor-pointer w-72 p-5 rounded-2xl border border-gray-200 transition flex flex-col hover:-translate-y-2 hover:shadow-2xl h-full"
    >
      ...
      <div class="flex flex-col pt-3 justify-between h-full">
        ...
        <p class="text-lg font-bold mt-auto ml-auto">{{ product.category }}</p>
      </div>
    </div>
  </NuxtLink>
</template>
